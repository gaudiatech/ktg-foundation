...
# import pyperclip
# NOTA BENE:  this replaces the genuine "pyperclip" lib that can link with the O.S.
class pyperclip:
    strv = None
...
class TextEditor:
    ...
    def display_scrollbar(self):
        ...
        pygame.draw.circle(self.screen, self.color_scrollbar, (int(x + (w / 2)), y + h),
                           int(w / 2))  # bottom round corner
    ...
    def __init__(self, offset_x, offset_y, text_area_width, text_area_height, screen, line_numbers_flag=False):
        ...
        # SCROLLBAR
        self.scrollBarImg = pygame.image.load('editor/myassets/Scroll_Bar.png').convert()
    ...
    def handle_keyboard_backspace(self) -> None:
        if self.chosen_LetterIndex == 0 and self.chosen_LineIndex == 0:
            ...
            self.line_string_list[self.chosen_LineIndex] = self.line_string_list[self.chosen_LineIndex] \
                                                               [:(self.chosen_LetterIndex - 1)] + self.line_string_list[
    ...
    def handle_keyboard_delete(self) -> None:
        if self.chosen_LetterIndex < (len(self.line_string_list[self.chosen_LineIndex])):
            ...
            self.line_string_list[self.chosen_LineIndex] = self.line_string_list[self.chosen_LineIndex] \
                                                               [:self.chosen_LetterIndex] + self.line_string_list[
                                                                                                self.chosen_LineIndex][
                                                                                            (self.chosen_LetterIndex + 1):]
...
# RQ: to inject text into the editor, plz use either:
...
def game_enter(vmstate=None):
    ...
    # editor.clear_text()
    editor.set_text_from_string(py_code)
