...
class TextEditorAsciiV(kengi.event.EventReceiver):
    def __init__(self, ref_mod, maxfps):
        ...
        self.codespace_nbcolumns = 78
        self.codespace_nbrows = -self.codestart_ij_pos[1]+(720//12)

        s = 12
        kengi.ascii.set_char_size(s)

        # -------
        # hackin the ol' model so we can use it without changes, but with the ASCIIkind of view
        # -------
        self._mod.yline_start_offset = s*self.codestart_ij_pos[1]  # bc it adds an offset for y coords on the screen
        self._mod.letter_size_X = s  # TODO should use a parameter instead of hacking, like this
        self._mod.letter_size_Y = s
        self._mod.line_gap = s
        self._mod.xline_start_offset = self.codestart_ij_pos[0] * s
    ...
    def _render_line_num(self, scr):
        if self._mod.displayLineNumbers:
            ...
            for num in range(self._mod.showStartLine, lastnum):
                nb_str_form = list('{: 4}'.format(num))
                for i, single_ch in enumerate(nb_str_form):
                    ascii_canvas.put_char(single_ch, [i, ybaseline + cpt], kengi.palettes.c64['lightgreen'])
                cpt += 1
    ...
    def _render_carret(self):
        ...
        ascii_canvas.put_char(kengi.ascii.CODE_FILL, cpos, self.colorpal['white'])
# ----------------------------
#  end editor view
# ----------------------------
...
def game_update(t_info=None):
    ...
    # print('pre-update manager >>>>>>>>>>>>>>')
    e_manager.post(lu_event)
    e_manager.post(paint_ev)
    # paint_ev.screen.fill('black')
    e_manager.update()
    kengi.flip()
