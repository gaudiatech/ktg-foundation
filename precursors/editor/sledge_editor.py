...
class EditorModel(kengi.event.CogObj):
    ...
    def _try_save_file(self, target_kart_id):
        ...
        if (target_kart_id is None) or self.locked_file:
            ...
            # show save icon:
            sharedstuff_obj.disp_save_ico = sharedstuff_obj.latest_t + SHOW_ICON_DURAT
            sharedstuff_obj.dump_content = self.fake_layout.dump_full_text()
...
class CapelloEditorView(kengi.event.EventReceiver):
    ...
    def _render_tabs(self, scr_ref):
        ...
        for k, tname in enumerate(self._mod.get_tabs_name()):
            ...
            if tname == selected_tname:
                pygame.draw.rect(scr_ref, self.lineNumberBackgroundColor, (r[0], r[1], r[2], r[3] + 2), 0)
                scr_ref.blit(self.img_editortab, (r[0], r[1]+1))
            ...
            x = r.center[0] - txtw // 2
            y = r.center[1] - txth // 2
            self.cfonts[2].text_to_surf(tname, scr_ref, (x, y+2), spacing=self.chosen_sp)
    ...
    def proc_event(self, ev, source=None):
        if ev.type == EngineEvTypes.PAINT:
            ...
            if self.scroll_dragging:
                if not (self.cycleCounter % self.CYCL_SCROLLING_FREQ):
        ...
        if self.rerenderLineNumbers and self._mod.displayLineNumbers:
            ...
            for x in range(binf, bsup):
                ...
                if x < self._mod.get_showable_lines():
                    ...
                    -4 + r[0] + r[2] - self.cfonts[0].compute_width(nstr, spacing=0), r[1] + self._mod.line_spacing),
                                                spacing=0)
                    # -old
...
class EditorCtrl(kengi.event.EventReceiver):
    ...
    def proc_event(self, ev, source=None):
        ...
        if ev.type == kengi.event.EngineEvTypes.LOGICUPDATE:
            sharedstuff_obj.latest_t = ev.curr_t
